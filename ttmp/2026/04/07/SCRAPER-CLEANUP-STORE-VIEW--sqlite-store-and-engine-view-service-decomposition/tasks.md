...
- [x] Extract leasing and heartbeat logic into `lease_store.go`.
...
- [x] Keep the public `Store` type stable during the move-only pass.
...
- [x] Keep `go test ./pkg/services/engineview ./pkg/api/server -count=1` green during the split.
- [x] Run `go test ./... -count=1` at the end.
- [x] Run `docmgr doctor --ticket SCRAPER-CLEANUP-STORE-VIEW --stale-after 30`.
