...
- [x] Add a small TS decode example or helper using `fromJson`

## Phase 2: Watermill Contract and Topics

- [x] Define Watermill topic names and message metadata conventions
- [x] Decide the delivery contract for replayable events vs best-effort transient events
...
- [x] Add Watermill Redis-backed transport for cross-process delivery
- [x] Add Docker Compose for local Redis development

## Phase 3: Worker and Submission Emission

- [x] Add worker-side event emission from the scheduler observer
- [x] Map scheduler events into generated `RuntimeEventV1` messages
- [x] Add runner/log event emission surfaces where worthwhile
- [x] Add server-side event emission for submission lifecycle visibility
- [x] Stop dropping submission-time workflow events on the `observer == nil` path

## Phase 4: Server Consumption and Frontend Delivery

- [x] Add a server-side Watermill subscriber/router for runtime events
- [x] Add a bounded recent-event buffer in the API server
- [x] Add server-side event history endpoint
- [x] Add server-side SSE endpoint
- [x] Decide how log payloads are stored: inline event payloads, artifacts, filesystem logs, or a mix
- [x] Add frontend decoding and state wiring for generated event messages

## Phase 5: Validation and Ops

- [x] Add integration tests that prove end-to-end event flow from submission through worker execution to frontend consumption
- [ ] Add a local single-process `server+worker` mode only if still useful after Redis-backed mode exists
- [x] Document operator workflows, message topics, and failure modes
