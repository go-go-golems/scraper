...
func registerCLI(root *cobra.Command) error {
	...
	runCmd := &cobra.Command{
		Use:   "run",
		Short: "Run a pure-JS demo workflow without HTTP fetches",
		RunE: func(cmd *cobra.Command, args []string) error {
			return runDemoWorkflow(cmd, options)
		},
	}
...
func runDemoWorkflow(cmd *cobra.Command, options *cliOptions) error {
	ctx := cmd.Context()
	if ctx == nil {
	...
	workflowParams, summaryOpID, err := BuildWorkflow(RunOptions{
	...
	result, err := engineStore.GetResult(ctx, workflow.ID, summaryOpID)
	if err != nil {
		return err
	}
	if result == nil {
		return fmt.Errorf("summary result %s not found", summaryOpID)
	}
	...
	_, _ = fmt.Fprintf(cmd.OutOrStdout(), "Summary op: %s\n", summaryOpID)
