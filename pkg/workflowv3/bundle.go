...
type BundleTask struct {
	...
	IsolationMaximum *IsolationPolicy  `json:"isolationMaximum,omitempty"`
}
...
func NewBundle(manifest BundleManifest, files map[string][]byte) (*Bundle, error) {
	...
	for _, task := range manifest.Tasks {
		...
		probe := TaskSpec{
			...
			BudgetMaximum: cloneBudgetClaim(task.BudgetMaximum), IsolationMaximum: isolationMaximumValue(task.IsolationMaximum),
		}
...
func (b *Bundle) Files() map[string][]byte {
	...
	ret := make(map[string][]byte, len(b.files))
	for name, body := range b.files {
		ret[name] = append([]byte(nil), body...)
	}
	return ret
}
...
func cloneManifest(manifest BundleManifest) BundleManifest {
	...
	for i, task := range manifest.Tasks {
		...
		ret.Tasks[i].IsolationMaximum = cloneIsolationPolicy(task.IsolationMaximum)
